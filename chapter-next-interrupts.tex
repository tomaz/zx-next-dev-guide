...
Both IM1 and IM2 are triggered by standard Spectrum ULA on every video frame. However timing can be adjusted using \PortLink{Raster Interrupt Control Register}{22} and \PortLink{Raster Interrupt Value Register}{23}. This allows disabling standard ULA interrupt, or add an extra interrupt that occurs on a particular video line.
...
Lines 4-6 enable or disable specific interrupters with \PortLink{Interrupt Enable 0-2}{C4-\$C6} registers.
...
With hardware IM2 mode it's possible to interrupt DMA operations. The choice of interrupters that can interrupt DMA is made with \PortLink{DMA Interrupt Enable 0-2}{CC-\$CE} registers. Here's what {\tt AA} wrote about this possibility on Next Discord server:
...
\subsubsection{Raster Interrupt Control Register \MemAddr{22}}

\begin{NextPort}
	\PortBits{7}
		\PortDesc{Read: $\mathtt{\overline{INT}}$ signal (even when Z80N has interrupts disabled) ({\tt 1} = interrupt is requested)}
...
\subsubsection{Video Line Interrupt Value Register \MemAddr{23}}
...
\subsubsection{Vertical Video Line Offset Register \MemAddr{64}}
...
\subsubsection{Interrupt Control \MemAddr{C0}}
...
If bit 3 is set, the return address pushed during an NMI acknowledge cycle will be written to \PortLink{NMI Return Address}{C2 \& \$C3} instead of the memory (the stack pointer will be decremented). The first {\tt RETN} after the NMI acknowledge will then take its return address from the registers instead of memory (the stack pointer will be incremented). If bit 3 is {\tt 0}, {\tt RETN} functions normally.


\subsubsection{NMI Return Address LSB \MemAddr{C2}}
\vspace*{-2ex}
\subsubsection{NMI Return Address MSB \MemAddr{C3}}
...
\subsubsection{Interrupt Enable 0 \MemAddr{C4}}

\begin{NextPort}
	\PortBits{7}
		\PortDesc{Expansion bus /INT ({\tt 1} after reset)}
	\PortBits{6-2}
...
\subsubsection{Interrupt Enable 1 \MemAddr{C5}}
...
\subsubsection{Interrupt Enable 2 \MemAddr{C6}}
...
\begin{NextPort}
	...
	\PortBits{1}
		\PortDesc{UART0 Rx near full}
	\PortBits{0}
		\PortDesc{UART0 Rx available}
\end{NextPort}

All bits {\tt 0} after reset. Rx near full overrides Rx available.


\subsubsection{Interrupt Status 0 \MemAddr{C8}}
...
\subsubsection{Interrupt Status 1 \MemAddr{C9}}
...
\subsubsection{Interrupt Status 2 \MemAddr{CA}}
