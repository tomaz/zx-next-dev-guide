...
Since core 3.1.2, zxnDMA is mapped to \PortLink{Datagear DMA Port}{xx6B} and Zilog DMA to \PortLink{MB02 DMA Port}{xx0B}.
...
\begin{DMADescription}
	...
	\DMADescriptionItem{4}{Ready Configuration}{
		\begin{DMAList}
			\item[{\tt 0}] \DMAPinLabel{CE}/\DMAPinLabel{WAIT} line functions only as chip enable line, allowing CPU to read and write control and status bytes when DMA is not bus master.
...
Core 3.1.2+ Always in Zilog DMA mode for uploading legacy DMA programs. For zxnDMA, use \PortLink{Datagear DMA Port}{xx6B}.

Older cores: this port is not supported, set bit 6 of \PortLink{Peripheral 2 Register}{06} and use \PortLink{Datagear DMA Port}{xx6B}.


\subsubsection{\PortTitle{Datagear DMA Port}{xx6B}}
