...
\begin{multicols}{2}
	...
	\DMAScaledReg{WR5}{
		\begin{tikzpicture}
			...
			\DMABitDescItem{4}{0}{$\mathtt{\overline{CE}}$ only}
			\DMABitDescItem{4}{1}{$\mathtt{\overline{CE}}$ and $\mathtt{\overline{WAIT}}$ multiplexed}
...
\DMARegName{WR0} specifies the direction of the transfer, the length of the data that will be transferred and inital port A configuration. Base register byte can be followed by up to four parameter bytes:
...
Whether the address represents memory or I/O port is defined with \DMARegName{WR1}.
...
\begin{DMADescription}
	...
	\DMADescriptionItem{3}{Port A Source}{
		...
		\begin{DMAList}
			...
			\item[{\tt 1}] Port A address is I/O.
		\end{DMAList}
	}

	\DMADescriptionItem{5-4}{Port A Address Handling}{
		The combination of the two bits determines how port A address will be handled after each byte is transferred:
...
\begin{DMADescription}
	...
	\DMADescriptionItem{3}{Port B\\Source}{
		...
		\begin{DMAList}
			...
			\item[{\tt 1}] Port B address is I/O.
		\end{DMAList}
	}
...
\begin{DMADescription}
	...
	\DMADescriptionItem{3-2}{Port B Starting Address}{
		Similar to bits 4-3 of \DMARegName{WR0}, these two bits indicate that port B address will follow. Bit 3 enables LSB and bit 2 MSB. Each byte can be enabled separately, though most commonly they are both used together. The address is interpreted either as memory or I/O port, based on the configuration from \DMARegName{WR2}.
	}
...
\begin{DMADescription}
	...
	\DMADescriptionItem{5}{Stop Configuration}{
		Specifies what happens when DMA reached the end of the block:
	...
	\DMADescriptionItem{4}{Ready Configuration}{
		\begin{DMAList}
			\item[{\tt 0}] $\mathtt{\overline{CE}}$/$\mathtt{\overline{WAIT}}$ line functions only as chip enable line, allowing CPU to read and write control and status bytes when DMA is not bus master.

			\item[{\tt 1}] This mode is implemented but currently not used in zxnDMA. This mode has an external device using the DMA's $\mathtt{\overline{CE}}$ pin to insert wait states during the DMA's transfer.
		\end{DMAList}
	}
...
\begin{tikzpicture}
	...
	% Read bytes
	\DMABitPar{0}{Status}[4em][pos=0.5][]
	...
	\begin{scope}
		\draw[PrintableDarkGray, line width=0.6pt]
			([xshift=15.5em,yshift=-1ex]\DMABitValID{0}.south east) -- 
			([xshift=-1.5em,yshift=-1ex]\DMABitValID{7}.south west)
			node (ReadDelimiterLine) {};

		% Read data text below delimiter
		\node[dmalegend, anchor=north east, rotate=90]
...
\begin{DMADescription}
	...
	\DMADescriptionByte{Read Mask Follows}{
		This is not one of the frequently used commands, but it's mentioned because it works differently: it indicates that parameter byte with reading mask will follow.
	}
...
This parameter provides a bit mask that defines which bytes will be read from DMA. While read mask parameter byte has to be written immediately after base register byte, as any other parameter, reading happens separately as I/O read from the DMA port \MemAddr{6B}.
...
\begin{DMADescription}
	...
	\DMADescriptionItem{3-4}{Port A Address}{
		Indicates that the current internal 16-bit port A address will be read from the I/O port. This is the last used address and will be used for the next transfer. Bit 3 indicates LSB and bit 4 MSB of the value will be read.
	}
...
\begin{itemize}[topsep=1pt,itemsep=1pt]
	...
	\item Line 22 has \DMARegName{WR5} register with bit 4 and 5 reset. Bit 4 tells DMA to use $\mathtt{\overline{CE}}$ only and bit 5 to stop at the end of the block, after all bytes are transferred.
	
	\item Line 24 uses \DMARegName{WR6} with bits 6-2 set to {\tt \%10011} which tells DMA to load source and destination addresses into its own internal pointers. This command must always be included at the end of the program, otherwise DMA internal pointers may point to old values.
	
	\item And finally, once everything is configured, line 25 includes another \DMARegName{WR6} with bits 6-2 set to {\tt \%00001}. This corresponds to ``Enable DMA''. After encountering this command, DMA will start running the program. Since we told it to ``load'' the pointers, it will start running at given start addresses as expected.
\end{itemize}
...
While the result is quite different, the two programs share a lot of similarities. Even more so if we consider reusing the same DMA program for uploading data to port \MemAddr{5B} from any memory address and of any size. As it stands now, we'd have to repeat the above program, only replace port A source and transfer length in lines 12-13.
...
It's also possible to program interrupters to interrupt DMA using Next specific Hardware IM2 mode. Which interrupters can interrupt DMA is configured with Next registers \PortLink{DMA Interrupt Enable 0-2}{CC-\$CE}. See chapter \ref{zx_next_interrupts} for more details on interrupts.


\subsection{DMA Registers}
\label{zx_next_dma_registers}

\subsubsection{Video Timing Register \MemAddr{11}}
...
\subsubsection{DMA Interrupt Enable 0 \MemAddr{CC}}
\vspace*{-2ex}
\subsubsection{DMA Interrupt Enable 1 \MemAddr{CD}}
\vspace*{-2ex}
\subsubsection{DMA Interrupt Enable 2 \MemAddr{CE}}
