...
However, when mode {\tt 11} is used (bits 7-6 of \PortLink{Copper Control High Byte}{62}, see section \ref{zx_next_copper_configuration} for details), Copper will auto-wrap to the first instruction on every vertical blank. This allows us to use {\tt HALT} to mark the end of the Copper program without having to fill in the remaining bytes with {\tt 0}. Quite convenient! In fact, I'd imagine this would be the most commonly used mode. If you're used to Copper on Amiga, this is also how it behaved there.
...
\subsubsection{\PortTitle{Copper Data}{60}}
...
The data is written to the index specified with \PortLink{Copper Control Low Byte}{61} and \PortLink{Copper Control High Byte}{62}. After the write, the index is auto-incremented to the next memory position. The index wraps to {\tt 0} when the last byte of the program memory is written to position {\tt 2047}. Since Copper instructions are 16-bits in size, two writes are required to complete each one.


\subsubsection{\PortTitle{Copper Control Low Byte}{61}}
...
\subsubsection{\PortTitle{Copper Control High Byte}{62}}
...
\subsubsection{\PortTitle{Copper Data 16-bit Write}{63}}
