...
Specific features of the Next are controlled via these register numbers, accessed via \PortLink{TBBlue Register Select}{243B}\footnote{\url{https://wiki.specnext.dev/TBBlue_Register_Select}} and \PortLink{TBBlue Register Access}{253B}\footnote{\url{https://wiki.specnext.dev/TBBlue_Register_Access}}, or via the {\tt NEXTREG} instruction.
...
Reading values from Next/TBBlue registers also occurs through \MemAddr{243B} and \MemAddr{253B} ports. Similar to write, read is also composed from 2 steps: first select the register with port \MemAddr{243B}, then read the value from port \MemAddr{253B}. For example reading a byte from port \MemAddr{B0}:
