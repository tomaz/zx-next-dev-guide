...
Next inherits ZX Spectrum keyboard handling, so all legacy programs will work out of the box. Additionally, it allows reading status of extended keys.


\subsection{Legacy Keyboard Status}

ZX Spectrum uses 8$\times$5 matrix for reading keyboard status. This means 40 distinct keys can be represented. Keyboard is read from \PortLink{ULA Control Port}{xxFE} with particular high bytes. There are 8 possible bytes, each will return status of 5 associated keys. If key is pressed, corresponding bit is set to {\tt 0} and vice versa.
...
Next uses larger 8$\times$7 matrix for keyboard, with 10 additional keys. By default, hardware is translating keys from extra two columns into existing 8$\times$5 set. However this can be turned off with bit {\tt 4} of \PortLink{ULA Control Register}{68}. Extra keys can be read separately via \PortLink{Extended Keys 0 Register}{B0} and \PortLink{Extended Keys 1 Register}{B1}.
...
Bits are reversed: if key is pressed, corresponding bit is {\tt 0}, if key is not pressed, bit is {\tt 1}.
